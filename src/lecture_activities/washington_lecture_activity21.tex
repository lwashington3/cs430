...
\begin{enumerate}
	...
	\answer{Insert and Extract-Max which are both $O(\lg n)$.}
	\item What if we needed a function to union two min binary heaps?
	How would you do it and what is the run time?
	\answer{You would have to call \Call{Extract-Max}{} on each element from the smaller heap and \Call{Insert}{} it into the other heap.}
	\item Why did we use an array for a binary heap?
	Does a heap need to be binary?
	\answer{Heaps match arrays well because both cannot have holes. It doesn't not need to be binary, it could be trinary and have a difference in calling the child nodes ($3n$, $3n+1$, $3n+2$) and the heap would be flatter, but at the tradeoff of \Call{Heapify}{} requiring more time.}
\end{enumerate}
		...
		\begin{tabular}{|l|l|}
			...
			\end{minipage} & \begin{minipage}{0.7\textwidth}
 				\begin{enumerate}
					...
					\answer{There are $2^{k}$ nodes.
					Its height is $k$.
					The root has $k$ children.}
					\item Why are they called binomial trees?
					\answer{The binomial numbers represent how many nodes there are on each level.}
 				\end{enumerate}
			\end{minipage}\\
			\bottomrule
		\end{tabular}
...
\begin{enumerate}[start=3]
	...
	\answer{You would need $\lg n$ binomial trees at most to make a binomial heap of $n$ nodes.}
...
\answer{Create a new $B_{0}$ heap and insert it at the front of the root list.
...
\answer{It would take $O(\lg n)$ to find the minimum value, and then you merge the children of the max back into the root list.}
