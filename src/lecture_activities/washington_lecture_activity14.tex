...
\begin{enumerate}[label=\arabic*.]
	...
	\footnote{Have an expoential problem space/solution space.}
	will work.\answer{\begin{itemize}
		\item Optimal Substructure\label{optimal-substructure} \begin{itemize}
	...
	\answer{Dynamic programming algorithms are optimization problems only, where divide and conquer algorithms could be used for more than optimization problems like sorting.
	...
	\answer{It matters which order we parenthesize a chain of matrix multiplications since the runtime might be larger for different configurations.
...
\answer{(Minimize the work in the matrix multiplication which depends on matrix dimensions.)}
\begin{enumerate}[label=\arabic*.]
	...
	\answer{Problem: $A_{1}\times A_{2}\times A_{3}\times\dots\times A_{n}$.
...
%	The base case is that \Call{P}{i,i}=0.
	...
	\end{minipage}} \begin{table}[H] %$#1\tablefootnote{#2}_{k=#3}$
        \centering
