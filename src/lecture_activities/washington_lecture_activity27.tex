...
\footnote{\begin{newanswer}Works on any graph with no negative cycles, and the algorithm finds the negative cycles.\end{newanswer}}
...
\begin{table}[H]
	...
	\begin{tabular}{|c|c|}
		...
		\begin{minipage}[t]{0.6\textwidth}
			\begin{algorithm}[H]
				\caption{Bellman-Form Shortest Path Algorithm \begin{newanswer}$O(VE)=O(V^{3})$\end{newanswer}}\label{alg:bellman-ford}
				\begin{algorithmic}[1]
				\Function{Bellman-Ford}{$V$, $E$, $w$, $s$}
					...
					\ForAll{edge $(u,v)\in E$}\State\Comment{All edges, in any order, same order each time}
						\If{$d[v] > d[u] + w(u,v)$}%
							\begin{newanswer}\Comment{Must be a negative cycle}\end{newanswer}
							\State \Return FALSE
						\EndIf
					\State \Return TRUE
					\EndFor
