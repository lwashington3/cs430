...
\begin{enumerate}[label=\arabic*.]
    \item In your own words explain how you insert a new key in a binary search tree. \oldanswer{When you have a value, you compare the value you're inserting to the value in the node. If the inserting value is less than the value, you recursively call the Insert function with the left node. If the value was greater, you recursively call Insert on the right node.}
	\item Give an example of a series of 5 keys inserted one at a time into a binary search tree that will yield a tree of height 5.\oldanswer{\\\globaltikzset
	\begin{tikzpicture}
		\Tree
		[.\node{ $5$ };
			\edge[blank];\node[blank]{ };
	...
	\begin{tikzpicture}
		...
		\end{tikzpicture}\oldanswer{\begin{tikzpicture}
		\Tree
		[.\node{ $Y$ };
			[.\node{ $X$ };
				[.\node{ $a$ }; ]
...
\begin{enumerate}[label=\arabic*)]
    \item Write pseudocode for BST Successor \oldanswer{(next larger value)} (or Predecessor). Demonstrate on the below tree from node 15 and then node 13.\\\bsttree\\\oldanswer{\begin{algorithm}[H]
    		...
    		\end{algorithmic}
    	\end{algorithm}}
	\item Write pseudocode for BST Insert. Demonstrate on the below tree to insert 5 and then 19.\\\bsttree\\\oldanswer{\begin{algorithm}[H]
			...
			\end{algorithmic}
		\end{algorithm}}
	\item What are the three possible cases when deleting a node from a BST?\\\bsttree\\\oldanswer{\begin{itemize}
		\item No children
		\begin{itemize}
		...
		\end{itemize}
	\end{itemize}}
	\item Write pseudocode for BST Delete (assume you already have a pointer to the node)\oldanswer{\begin{algorithm}[H]
...
\begin{enumerate}[label=\arabic*.,start=5]
    \item Write pseudocode for LeftRotate (or RightRotate). What is the worst case runtime?\oldanswer{\begin{algorithm}[H]
