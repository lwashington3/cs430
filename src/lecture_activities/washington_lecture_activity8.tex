...
\section{Opening Questions}\label{sec:opening-questions-8}
\begin{itemize}
	\item If we have 17 distinct items to sort, what is the height of the decision tree that represents all possible orderings of the 17 items?
\end{itemize}
...
\begin{enumerate}[label=\arabic*.]
    \item How many possible permutations of $n$ items are there (at least one of them must be sorted)? In the decision tree model, how many leaves are there for $n$ items? For a binary tree to have that many leaves, how many levels does it need? Why do we care about how many levels are in the decision tree?
\end{enumerate}
...
\begin{itemize}
	\item Counting sort assumes that each of the $n$ input elements is an integer in the range $0$ to $k$, for some integer $k$. When $k=O(n)$, the sort runs in $T(n)$ time (best if $k \lll n$)
