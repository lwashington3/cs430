...
\answer{$O(1)$ Makes a single node heap.}

\binomialqueueop{Insert}
\answer{$O(1)$ Make single node heap, put in the (bidirectional linked) list to left of current min.
Update min pointer if necessary.}

\binomialqueueop{Minimum}
\answer{$O(1)$ We maintain a pointer to the min.}

\binomialqueueop{Union}
\answer{$O(1)$ Link the two root lists together%
...
\answer{$O(1)$ Reduce value at that node, move it and its subtree to root list, check if new min value pointer.
