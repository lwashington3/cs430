...
\begin{enumerate}[label=\arabic*.]
	...
	\begin{enumerate}[label=\choicelabel]
		...
		\item \answer{perform a post-order traversal of the order-statistic tree and sum the sizes of each subtree of a node and add one to get the size of each node (nodes with no children assigned size=1)}
	\end{enumerate}
	...
	\begin{enumerate}[label=\choicelabel]
	    ...
	    \item \answer{Augmented data structures offer additional operations or information.}
	...
	\begin{enumerate}[label=\choicelabel]
	    \item \answer{Overlapping subproblems}
	...
	\item Let $R(i,j)$ be the number of times that table entry $m[i,j]$ is referenced while computing other table entries in a call of MATRIX-CHAIN-ORDER. Show that the total number of references for the entire table is \[ \sum_{i=1}^{n}\sum_{j=i}^{n} R(i,j) = \frac{n^{3} - n}{3} \] \answer{\begin{equation*}
	\begin{aligned}
