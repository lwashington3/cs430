...
\begin{enumerate}[label=\arabic*.]
	...
	\begin{enumerate}[label=\alph*)]
		...
		\item \answer{\textbf{Statement 1 is true but statement 2 is false}. There may be a smaller value $>m$ indexes from the start of the array, which is why statement 2 is false, however, the first $m$ elements would be sorted amongst themselves.}
...
\end{enumerate}
	...
	\begin{enumerate}[label=\alph*)]
	    ...
	    \item \answer{$\mathbf{O\left( n^{2} \right)}$ The use of binary search reduces the time of finding the correct position from $O(n)$ to $O(\log n)$. But the worst case of insertion sort remains $O(n^{2})$ because of the series of swaps required for each insertion.}
	\end{enumerate}
	...
	\begin{enumerate}[label=\arabic{enumi}\alph*)]
	    \item What is the big-O runtime for the routine, measured as a function of its return value $i$? \answer{$O(n) \rightarrow O(i)$.} \correction{$T(i)=ai+b=O(i)$ ($a$ and $b$ are constants).}
		\item If the numbers are chosen independently at random, then the probability that firstDecrease($L$) returns $i$ is $\frac{i-1}{i!}$, except for the special case of $i=n+1$ for which the probability is $\frac{1}{n!}$ Use this fact to write an expression for the expected value returned by the algorithm. (Your answer can be expressed as a sum, it does not have to be solved in closed form. Do not use O-notation.) Use expectation \answer{\begin{equation*}
		\begin{aligned}
	...
	\begin{enumerate}[label=\choicelabel]
	    \item\label{prb:7a} Use pseudocode to specify a brute-force algorithm that determines when a sequence of $n$ positive integers is given as input, whether there are two distinct terms of the sequence that have as sum a third term. The algorithm should loop through all triples of the sequence, checking whether the sum of the first two terms equals the third.\answer{\begin{algorithm}[H]
	    	...
	    	\end{algorithmic}
	    \end{algorithm}}
		\item Give a big-O estimate for the complexity of the brute-force algorithm from \hyperref[prb:7a]{part (a)}.\answer{$O(n) \times O(n) \times O(n) = O(n^{3})$}
		\item Devise a more efficient algorithm for solving the problem that first sorts the input sequence and then checks for each pair of terms whether their sum is in the sequence.\answer{\begin{algorithm}[H]
